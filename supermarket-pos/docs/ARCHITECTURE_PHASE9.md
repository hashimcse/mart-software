...
## Returns: proportional refunds, and returnability tracked across repeated partial returns

A return's refund per line is `(saleItem.subtotal + saleItem.tax) / saleItem.quantity × returnQuantity` — the amount actually paid per unit (already net of that line's discount, and inclusive of its share of tax whether the tax was inclusive or exclusive in the original sale), scaled to however many units are being returned. That's what makes a return of "2 of 5 discounted, taxed units" (spec section 38's example) refund the right amount instead of the list price.

Preventing over-returning has to hold across *multiple* partial returns on the same sale, not just the first one — a customer returning 2 today and 2 more next week shouldn't be able to return more than the 1 that's actually left. `return.service.ts` sums every prior `ReturnItem` for a sale item before allowing a new return against it, inside the same transaction that creates the new one. The sale's status (`PARTIALLY_RETURNED` vs `RETURNED`) is recomputed from that same running total across every line, not just the line being returned right now.

Each returned line independently chooses whether to restock (spec's "restore inventory where appropriate" — a defective item shouldn't go back on the shelf). Purchase returns don't get that choice: the goods are physically leaving the store back to the supplier either way, so stock always decreases there, through the same race-safe guarded decrement as sales.

## Cash register: the Phase 4 "tendered vs applied" question, resolved

Phase 4 deliberately recorded `Payment.amount` as the amount *applied* to a sale, not the gross amount a customer handed over — change given back was never stored. At the time this was flagged as something Phase 9 would need to reconcile. It turns out no reconciliation is needed: for cash, `tendered − change = applied` is just algebra, which means recording a single cash movement of `applied` on a sale gives the *exact* correct net effect on the drawer as recording gross tendered-in and change-out separately would. Verified this equivalence directly (`tendered=1000, change=200` on an `800` sale nets to the same `800` either way) rather than assuming it. So `cashSession.service.ts` never needed a new "tendered" field — it just reads the same `Payment.amount` every other part of the app already uses.

A sale or cash refund posts to whichever terminal has an open session automatically, inside the same transaction — linking is opportunistic, not required, so a terminal without an open register still rings up sales exactly as it did in every prior phase.
...
**Frontend — fully verified.** `npm install`, `tsc --noEmit`, and `vite build` all run clean with no errors, across all nine phases built so far. The login flow, token-refresh-on-401 logic, protected routing, product search/pagination/image-upload, the POS cart/checkout/hold-resume flow, split-payment entry, Settings, Inventory, Purchases/Suppliers, Customers/loyalty, and the return flows (sale returns from the POS screen, purchase returns from the purchase detail modal) are real code exercised by the type checker and bundler, not mocked. The cart's live tax-preview math (`lib/cartMath.ts`) was additionally run standalone against hand-computed expected values to confirm it agrees with the backend's formula.
...
- The return/refund math — checked against 13 scenarios including proportional partial-return refunds on a discounted+taxed line, the over-return cap holding across repeated partial returns, correct PARTIALLY_RETURNED vs RETURNED sale-status transitions, and the per-sale-capped credit-balance offset from a return.
- The cash register math — checked the tendered-vs-applied equivalence directly, plus a full shift's expected-cash computation across sales, refunds, cash in/out, and expenses.
