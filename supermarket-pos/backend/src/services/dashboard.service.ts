...
export async function getDashboardSummary() {
  ...
  const [todaySales, lowStock, outOfStock, creditPayments, customerPayments] = await Promise.all([
  ...
  return {
    todaySalesTotal: (todaySales._sum.total ?? new Prisma.Decimal(0)).toFixed(2),
